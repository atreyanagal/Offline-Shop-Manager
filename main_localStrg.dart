...
class _TodoAppState extends State<TodoApp> {
  ...
  void _toggleTheme() async {
    ...
    setState(() {
      _currentTheme = isDarkTheme ? ThemeData.light() : ThemeData.dark();
...
// Calculate total sales till now
  ...
  void updateTodoItem(Todo editedItem) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String> todosJson = prefs.getStringList('todos') ?? [];
    final index = todos.indexWhere((todo) => todo.title == editedItem.title);
    if (index != -1) {
      int quantityDifference = editedItem.quantity - todos[index].quantity;

      // If quantity is reduced, update the sales till now
      if (quantityDifference < 0) {
        double salesAmount = editedItem.cost * quantityDifference.abs();
        totalSales -= salesAmount;
      }

      todos[index] = editedItem;
      todosJson[index] = json.encode(editedItem.toJson());
